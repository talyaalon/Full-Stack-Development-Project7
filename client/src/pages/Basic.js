...
export default function Basic() {
  ...
  const [loadedBasics, setLoadedBasics] = useState([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch(`http://localhost:3000/basic`);
        const data = await response.json();
        setLoadedBasics(data);
      } catch (error) {
        console.error(error);
      }
    }
  ...
  const handleAddToShoppingCart = async (product) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/basic/${product.id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (response.ok) {
        ...
        console.log(data);
        setLoadedBasics((prevBasics) =>
          prevBasics.map((basic) => (basic.id === product.id ? data : basic))
        );
      } else {
        console.error("Error updating basic:", response.status);
      }
    } catch (error) {
  ...
  const handleDeleteProduct = (id) => {
    try {
      fetch(`http://localhost:3000/basic/${id}`, {
        method: "DELETE",
      });

      // Update the local state to remove the deleted product
      setLoadedBasics((prevProducts) =>
  ...
  const handleSaveChanges = async (id) => {
    try {
      const response = await fetch(`http://localhost:3000/basic/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        setLoadedBasics((prevBasics) =>
          prevBasics.map((basic) => (basic.id === id ? updatedProduct : basic))
        );
      } else {
        console.error("Error updating basic:", response.status);
      }
  ...
  const handleSaveNewProduct = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("new product:");
        console.log(data);
        setLoadedBasics((prevBasics) => [...prevBasics, data]);
      } else {
  ...
  return (
    <div className="photos-container" lang="he" dir="rtl">
      ...
      <div className="photo-grid">
        {loadedBasics.map((basic) => (
          <div
            className="photo-item"
            key={basic.id}
            onClick={() => openProductModal(basic)}
          >
            <div className="photo-frame">
              <img src={basic.image_url} alt={basic.name} />
            </div>
            <div className="photo-overlay">
              <span className="photo-title">{basic.name}</span>
              <br></br>
              <span className="photo-title">מחיר: {basic.price}₪</span>
            </div>
          </div>
        ))}
      </div>
