...
export default function Frozen() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch(`http://localhost:3000/frozen`);
  ...
  const handleAddToShoppingCart = async (product) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/frozen/${product.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDeleteProduct = (id) => {
    try {
      fetch(`http://localhost:3000/frozen/${id}`, {
        method: "DELETE",
      });
  ...
  const handleSaveChanges = async (id) => {
    try {
      const response = await fetch(`http://localhost:3000/frozen/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
