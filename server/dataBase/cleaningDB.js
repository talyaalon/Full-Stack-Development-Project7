...
function getCleanings(callback) {
...
function updateCleaning(callback, id, body) {
  console.log(body);
  const { name, price, category, quantity, image_url } = body;

  let sql = `UPDATE cleaning SET`;
  const values = [];
  ...
  databaseConnection.query(sql, values, (error, results, fields) => {
    ...
    // Fetch the updated Cleaning from the database
    const selectSql = `SELECT * FROM cleaning WHERE id = ?`;
...
function deleteCleaning(callback, id) {
  const selectSql = `SELECT * FROM cleaning WHERE id = ?`;
  databaseConnection.query(selectSql, [id], (selectError, selectResults) => {
    if (selectError) throw selectError;
    else {
  ...
  let sql = "DELETE FROM cleaning WHERE id = ?";
  databaseConnection.query(sql, [id], function (err, result, fields) {});
}

function addCleaning(callback, body) {
  console.log(body);
  const { name, price, category, quantity, image_url } = body;

  const sql =
    "INSERT INTO cleaning (name, price, category, quantity, image_url) VALUES (?, ?, ?, ?, ?)";
  databaseConnection.query(
    sql,
...
module.exports = { getCleanings, updateCleaning, deleteCleaning, addCleaning };
